  ...
  callbackFunction() {
    ...
    this.editorObject.images_dataimg_filter = (img) => {
      if (
        ...
        img.getAttribute('class').indexOf(Configuration.get('imageClassName')) !== -1
      ) {
...
*/
;(function () {
  tinymce.PluginManager.add('tiny_mce_wiris', (editor, url) => ({
    ...
    init(editor) {
      ...
      if (typeof editor.getParam('wiriscontextpath') !== 'undefined') {
        integrationModelProperties.configurationService = Util.concatenateUrl(
          editor.getParam('wiriscontextpath'),
          integrationModelProperties.configurationService
        )
      ...
      const onInit = function (editor) {
        ...
        editor.setContent(Parser.initParse(content, editor.getParam('language')), {
          format: 'html',
        })
